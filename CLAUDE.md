...
[AIアバター](docs/GLOSSARY.md#avatar)がユーザーとリアルタイムでインタラクティブな会話を行う**[マルチテナント](docs/GLOSSARY.md#multi-tenant-マルチテナント)型SaaS**プラットフォーム。事前設定された[シナリオ](docs/GLOSSARY.md#scenario)に基づき、AIアバターが自律的に会話を進め、その様子を録画・解析・レポーティングする。

**マルチテナント型SaaSの特徴:**

- 単一のインフラで複数の組織（[テナント](docs/GLOSSARY.md#tenant-テナント)）を完全分離して管理
...
interface TTSProvider extends AIProvider {
  synthesizeSpeech(text: string, voiceId: string, options: TTSOptions): Promise<AudioStream>;
...
class ProviderManager {
  ...
  async switchProvider(type: ProviderType, providerName: string): Promise<void> {
    // プロバイダの切り替えロジック
    // - 新プロバイダの初期化
    // - 既存セッションへの影響評価
    // - グレースフルな移行
...
async function calculateBenchmark(
  ...
  // 7. 改善提案生成（Claude API）
  const recommendations = await generateRecommendations(userMetrics, orgBenchmark, userSessions);
...
async function checkRateLimit(apiKeyId: string, limits: RateLimits): Promise<boolean> {
  const now = Date.now();
...
- 新しい言語を追加する際、ソースコードの変更・再ビルド・再デプロイは一切不要
- 言語リソースファイル（JSON）を追加するだけで即座に新言語対応完了
- スーパー管理者UIから言語リソースをアップロード → ホットデプロイ

**2. 表示テキストの完全分離**

- **全ての表示用テキストは言語リソースファイルに格納**
- **ソースコードに表示用文字列は一切含まない**
- ハードコードされた文字列は厳密に禁止（Linterで自動検出）

**3. 統一化された管理手段**

- 言語選択、デフォルト言語設定、フォールバック設定を統一されたシステムで管理
- フロントエンド・バックエンド・API・メール通知すべてで同一の言語リソースを使用
- 言語リソースのバージョン管理・ロールバック機能

**4. ホットデプロイの実現**

- 言語リソースの変更がリアルタイムに全ユーザーに反映
- キャッシュの自動無効化（CloudFront + Redis）
- デプロイ前のプレビュー・検証機能

#### 言語リソース管理システム（スーパー管理者専用）

##### 言語リソースアップロードUI

```
┌──────────────────────────────────────────────────────────────┐
│ 言語リソース管理 (スーパー管理者専用)       [+ 新しい言語追加] │
├──────────────────────────────────────────────────────────────┤
│                                                               │
│ 📊 サポート言語一覧                                           │
│ ┌────────────────────────────────────────────────────────┐   │
│ │ ✅ 日本語 (ja)                    [編集] [プレビュー]   │   │
│ │    ステータス: アクティブ                               │   │
│ │    最終更新: 2026-03-04 10:30     更新者: admin        │   │
│ │    バージョン: v1.2.5             キー数: 1,247        │   │
│ │    完成度: ██████████ 100%        欠落キー: 0          │   │
│ ├────────────────────────────────────────────────────────┤   │
│ │ ✅ English (en)                   [編集] [プレビュー]   │   │
│ │    ステータス: アクティブ                               │   │
│ │    最終更新: 2026-03-03 14:20     更新者: admin        │   │
│ │    バージョン: v1.2.5             キー数: 1,247        │   │
│ │    完成度: ██████████ 100%        欠落キー: 0          │   │
│ ├────────────────────────────────────────────────────────┤   │
│ │ 🟡 中文简体 (zh-CN)               [編集] [プレビュー]   │   │
│ │    ステータス: ドラフト                                 │   │
│ │    最終更新: 2026-02-28 09:15     更新者: translator   │   │
│ │    バージョン: v1.1.0             キー数: 892          │   │
│ │    完成度: ████████░░ 71%         欠落キー: 355 [表示] │   │
│ │    [公開する]                                          │   │
│ └────────────────────────────────────────────────────────┘   │
│                                                               │
...
│                                                               │
│ ステップ1: 言語情報                                           │
│ 言語コード:    [zh-CN         ] (ISO 639-1)                  │
│ 表示名:        [中文简体      ] (日本語)                      │
│ 表示名:        [Simplified Chinese] (English)                │
│ ネイティブ名:  [简体中文      ]                               │
│ 方向:          ○ LTR (左→右)   ○ RTL (右→左)                │
│                                                               │
│ ステップ2: リソースファイルアップロード                       │
│ 方法選択:                                                     │
│   ○ JSONファイル一括アップロード (推奨)                      │
│   ○ 基準言語からコピー（後で編集）                           │
│   ○ 空のテンプレートを生成                                   │
...
│ │                                                        │    │
│ │ 対応形式: JSON, YAML                                   │    │
│ │ 推奨構造: locales/{locale}/*.json                      │    │
│ └───────────────────────────────────────────────────────┘    │
│                                                               │
│ アップロードされたファイル:                                   │
│ ✅ common.json        (245 keys)                             │
│ ✅ dashboard.json     (128 keys)                             │
│ ✅ scenarios.json     (89 keys)                              │
│ ✅ reports.json       (156 keys)                             │
│ ✅ errors.json        (93 keys)                              │
│ ⚠️  emails.json       (欠落) ← 必須ファイル                  │
│                                                               │
│ ステップ3: 検証とプレビュー                                   │
│ [検証実行]                                                    │
│                                                               │
...
│    - dashboard.user_profile_title                            │
│    - ...                                                     │
│ ❌ 必須ファイル欠落: emails.json                              │
│                                                               │
│ [プレビュー] [ドラフトとして保存] [公開]                      │
└──────────────────────────────────────────────────────────────┘
```
...
# リソースファイル例: locales/ja/common.json
{
  "app": {
    "name": "Prance",
    "tagline": "AIアバター会話プラットフォーム"
  },
  "nav": {
    "dashboard": "ダッシュボード",
    "sessions": "セッション",
...
# "session_duration": "{minutes}分{seconds}秒"
```
...
│    ↓                                                          │
│ 3. S3にバージョン付きで保存                                   │
│    s3://locale-resources/{locale}/{version}/common.json      │
│    ↓                                                          │
│ 4. DynamoDB: メタデータ更新                                   │
│    - 最新バージョン番号                                       │
│    - キー数、完成度                                           │
...
│    Invalidate /locales/{locale}/*                            │
│    ↓                                                          │
│ 7. EventBridge: 通知イベント発行                              │
│    event: locale.updated                                     │
│    ↓                                                          │
│ 8. 全アプリケーションがリロード時に新リソースを取得          │
│    - フロントエンド: 次回ページロード時                       │
│    - バックエンド: 次回リクエスト時                           │
│    - Lambda: 次回実行時（環境変数でバージョン指定）           │
│                                                               │
│ ⏱️  反映時間: 数秒〜1分以内                                   │
└──────────────────────────────────────────────────────────────┘
```
...
```typescript
// packages/shared/src/i18n/loader.ts
...
export async function loadLocaleResources(
  ...
  const cacheKey = `locale:${locale}:${namespace}`;
  ...
  try {
    ...
    // 2. S3から最新バージョン取得
    const version = await getLatestVersion(locale);
    const s3Key = `${locale}/${version}/${namespace}.json`;

    const response = await s3.send(
      new GetObjectCommand({
        Bucket: process.env.LOCALE_RESOURCES_BUCKET,
        Key: s3Key,
      })
    );
    ...
    // 3. Redisにキャッシュ（TTL: 1時間）
    await redis.setEx(cacheKey, 3600, JSON.stringify(resources));

    return resources;
  } catch (error) {
    console.error(`Failed to load ${locale}/${namespace}:`, error);

    // 4. フォールバック: 英語リソースを読み込み
    if (locale !== 'en') {
      console.warn(`Falling back to English for ${namespace}`);
      return loadLocaleResources('en', namespace);
    }
 ...
 * 最新バージョン番号を取得
 */
async function getLatestVersion(locale: string): Promise<string> {
  // DynamoDBから最新バージョンを取得
  const result = await dynamodb.getItem({
    TableName: 'locale_metadata',
    Key: { locale: { S: locale } },
  });

  return result.Item?.latest_version?.S || 'v1.0.0';
}
```

##### フロントエンド実装（コード内に文字列なし）
...
function ItemCount({ count }: { count: number }) {
  ...
  return <span>{t('item_count', { count })}</span>;
}
// locales/ja/common.json:
// "item_count": "{count}個のアイテム"
// "item_count_zero": "アイテムはありません"
// "item_count_one": "1個のアイテム"
// "item_count_other": "{count}個のアイテム"
```

##### バックエンドAPI実装（コード内に文字列なし）
...
app.get('/api/sessions/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(404).json({
      error: await t('api.errors.session_not_found', { locale }),
...
// 翻訳ヘルパー関数
async function t(key: string, options: { locale: string; params?: Record<string, any> }) {
  const [namespace, ...keyPath] = key.split('.');
  const resources = await loadLocaleResources(options.locale, namespace);
  ...
  if (!value) {
    console.warn(`Missing translation: ${key} (${options.locale})`);
    return key;
  }
...
```typescript
// eslint-plugin-i18n-keys.js
// 全ての表示用文字列がi18nキーから読み込まれていることを検証
      ...
      create(context) {
        return {
          JSXText(node) {
            ...
            // 空白文字のみ、または数値のみは許可
            if (text && !/^\s*$/.test(text) && !/^\d+$/.test(text)) {
              context.report({
                node,
                message: `Hardcoded string detected: "${text}". Use i18n key instead.`,
              });
            }
          },
          Literal(node) {
            // 特定の関数呼び出し内の文字列リテラルを検出
            const parent = node.parent;
            const isJSXAttribute = parent.type === 'JSXAttribute';
            const isI18nCall = parent.callee?.name === 't' || parent.callee?.name === 'useTranslations';

            if (!isI18nCall && typeof node.value === 'string') {
              const text = node.value.trim();
              // URLや特殊な文字列は除外
              if (text && !/^(https?:\/\/|data:|#|\.\/|@)/.test(text)) {
                context.report({
                  node,
                  message: `Hardcoded string detected: "${text}". Use i18n key instead.`,
                });
...
// .eslintrc.json
{
  "plugins": ["i18n-keys"],
  "rules": {
    "i18n-keys/no-hardcoded-strings": "error"
  }
}
```
...
class GreenhouseAdapter implements ATSAdapter {
  async createScorecard(data: ScorecardData): Promise<string> {
    const response = await fetch(`${this.baseUrl}/scorecards`, {
      method: 'POST',
      headers: {
...
// プラグインサンドボックス実行
class PluginSandbox {
  async execute(plugin: Plugin, method: string, args: any[], context: PluginContext): Promise<any> {
    // 1. 権限チェック
...
# ============== 多言語対応（新規） ★NEW ==============
locale_resources ★NEW
  id, locale(ja/en/zh-CN/etc.), namespace(common/dashboard/scenarios/etc.)
  version, status(draft/active/deprecated)
  s3_key, content_hash, key_count, completeness_percentage
  missing_keys_json, updated_by(FK), created_at, updated_at
  published_at

  # 例:
  # locale='ja', namespace='common', version='v1.2.5'
  # s3_key='ja/v1.2.5/common.json'
  # status='active', key_count=1247, completeness_percentage=100

locale_metadata ★NEW
  id, locale(ja/en/zh-CN/etc.)
  display_name_ja, display_name_en, display_name_native
  is_active, is_rtl(boolean)
  latest_version, total_keys, avg_completeness
  usage_count_30d, created_at, updated_at

  # 例:
  # locale='ja', display_name_ja='日本語', display_name_native='日本語'
  # is_active=true, latest_version='v1.2.5', total_keys=1247
...
# 言語リソース管理（スーパー管理者専用） ★NEW
GET    /api/super-admin/locale-resources               # 全言語リソース一覧
POST   /api/super-admin/locale-resources               # 新しい言語リソース追加（ファイルアップロード）
GET    /api/super-admin/locale-resources/:locale/:namespace  # 特定リソース取得
PUT    /api/super-admin/locale-resources/:locale/:namespace  # リソース更新
DELETE /api/super-admin/locale-resources/:locale/:namespace  # リソース削除
POST   /api/super-admin/locale-resources/:locale/:namespace/publish  # ドラフトを公開
POST   /api/super-admin/locale-resources/:locale/:namespace/rollback # 前バージョンにロールバック
GET    /api/super-admin/locale-resources/:locale/:namespace/versions # バージョン履歴
POST   /api/super-admin/locale-resources/validate      # リソースファイル検証（アップロード前）
GET    /api/super-admin/locale-resources/missing-keys  # 欠落キー一覧
GET    /api/super-admin/locale-metadata                # 言語メタデータ一覧
POST   /api/super-admin/locale-metadata                # 新しい言語メタデータ作成
PUT    /api/super-admin/locale-metadata/:locale        # 言語メタデータ更新
POST   /api/super-admin/locale-cache/invalidate        # キャッシュ無効化（ホットデプロイ）
```
...
- [ ] 多言語対応システム ★NEW
  - [ ] **言語リソース管理システム**（コード変更不要・ホットデプロイ対応）
    - [ ] データベーステーブル作成（locale_resources, locale_metadata）
    - [ ] S3バケット構成（バージョン管理対応）
    - [ ] Lambda: リソースローダー実装（S3 + Redis キャッシュ）
    - [ ] Lambda: バリデーション関数（JSON構文、必須キー確認）
    - [ ] EventBridge: キャッシュ無効化イベント
    - [ ] スーパー管理者UI: リソースアップロード画面
    - [ ] スーパー管理者UI: リソース一覧・編集・プレビュー画面
    - [ ] ホットデプロイメカニズム（CloudFront + Redis キャッシュ無効化）
    - [ ] フォールバック処理（翻訳欠落時の自動対応）
    - [ ] バージョン管理・ロールバック機能
    - [ ] ESLint プラグイン（ハードコード文字列検出）
  - [ ] i18n実装（フロントエンド: next-intl）
    - [ ] 全コンポーネントでi18nキー使用（文字列ハードコード禁止）
    - [ ] 変数展開、複数形対応
  - [ ] バックエンドi18n（i18next）
    - [ ] APIレスポンス多言語化（Accept-Language対応）
    - [ ] 翻訳ヘルパー関数実装
  - [ ] 翻訳データベース設計・実装（エンティティ別翻訳）
  - [ ] 言語切り替えUI（ヘッダー、ユーザー設定）
  - [ ] 日本語・英語リソースファイル作成（全10ファイル × 2言語）
    - [ ] common.json, dashboard.json, auth.json, scenarios.json
    - [ ] sessions.json, reports.json, admin.json, errors.json
    - [ ] emails.json, api.json, validation.json
  - [ ] シナリオ多言語対応（システムプロンプト）
  - [ ] レポート多言語対応（テンプレート、AI生成フィードバック）
  - [ ] メール通知多言語対応
...
- [ ] 多言語拡張（中国語、フランス語、ドイツ語、スペイン語等）
  - スーパー管理者UIからリソースファイルをアップロードするだけ
  - コード変更・再ビルド・再デプロイ一切不要
  - ホットデプロイで即座に新言語対応完了
- [ ] ATS連携拡大 (追加プロバイダ)
...
_本企画書は詳細設計フェーズ v2.0 の内容です。実装進行に伴い随時更新されます。_
