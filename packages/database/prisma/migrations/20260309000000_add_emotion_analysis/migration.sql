...
-- AddForeignKey (IF NOT EXISTS to avoid errors on re-run)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'emotion_analyses_session_id_fkey') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'emotion_analyses_recording_id_fkey') THEN
        ALTER TABLE "emotion_analyses" ADD CONSTRAINT "emotion_analyses_recording_id_fkey"
        FOREIGN KEY ("recording_id") REFERENCES "recordings"("id") ON DELETE SET NULL ON UPDATE CASCADE;
    END IF;
END $$;
