...
console.log(`[session_end] Sorted chunks (first 5):`, sortedChunks.slice(0, 5).map(c => c.Key));
```
...
console.log('[VideoProcessor] Sorted chunks (first 5):', sortedChunks.slice(0, 5).map(c => c.Key));
```
