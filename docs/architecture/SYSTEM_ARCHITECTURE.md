...
export function useRealtimeSession(sessionId: string) {
  ...
  return {
    ...
    sessionState,
    controls: {
      toggleMicrophone: () => setSessionState((prev) => ({ ...prev, microphoneActive: !prev.microphoneActive })),
...
// カスタム属性（マルチテナント対応）
userPool.addCustomAttribute('tenantId', new cognito.StringAttribute({
  mutable: false,
}));
userPool.addCustomAttribute('role', new cognito.StringAttribute({
  mutable: true,
}));
```
