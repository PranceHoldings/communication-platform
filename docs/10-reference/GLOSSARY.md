...
Route 53で管理するプラットフォーム専用ドメイン。`platform.prance.jp`がサブドメイン委譲される。
...
本プラットフォームで使用するドメインレジストラ。`prance.jp`を管理。
...
`platform.prance.jp`のみをRoute 53に委譲し、お名前.comでの変更を最小限に抑える設計方式。
