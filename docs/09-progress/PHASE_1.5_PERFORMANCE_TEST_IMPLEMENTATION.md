...
3. **認証トークン取得スクリプト** (`scripts/get-auth-token.sh`)
...
**ファイル:** `scripts/get-auth-token.sh`
**サイズ:** 35行
**機能:**
- Lambda APIへのログイン
- JWTアクセストークン取得
...
```bash
cd infrastructure
npm run cdk -- deploy Prance-dev-Monitoring
```
...
```json
{
  "scripts": {
    ...
    "perf:load": "npx tsx scripts/performance-test.ts --load 10",
