...
```bash
# 現在の構造（問題あり）
Step 1: npm run lambda:predeploy  # 手動実行
Step 2: npx cdk deploy            # 手動実行（検証をスキップ可能）
```
...
1. `npx cdk deploy Prance-dev-ApiLambda`
2. `npm run cdk -- deploy Prance-dev-ApiLambda`
3. `npm run deploy`
4. `npm run deploy:dev`
5. `npm run deploy:staging`
6. `npm run deploy:production`
7. `npm run deploy:quick`
8. `./deploy.sh dev`

**問題:**
- どれを使えば検証されるのか不明瞭
- 直接`npx cdk`を実行すると、すべての検証をバイパス
...
|------|-----------|------|
| 13:00 | マイグレーション実行 | ✅ 正常 |
| 13:05 | Lambda関数をデプロイ | ❌ `npx cdk deploy`を直接実行 |
...
```json
{
  ...
  "cdk": "echo '⚠️  WARNING: Use npm run deploy' && cdk"
}
```
...
1. **直接CDKコマンド実行 → 警告表示**
   ```bash
   npm run cdk
   # ⚠️  WARNING: Use npm run deploy instead
   ```
