...
検証コマンド: pnpm run validate:ui-settings
...
**統合済みコマンド:**
- `pnpm run validate:ui-settings` - UI設定項目同期検証
- `pnpm run pre-deploy` - デプロイ前全検証（Check 13含む）
...
- name: Validate UI Settings Sync
  run: pnpm run validate:ui-settings
```
...
```bash
# 追加したフィールドを検証
pnpm run validate:ui-settings -- --field <fieldName>
```
...
2. コミット前に `pnpm run validate:ui-settings` を実行
3. デプロイ前に `pnpm run pre-deploy` を実行
