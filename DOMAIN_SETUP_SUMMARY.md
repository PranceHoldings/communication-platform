...
| 環境 | ドメイン | 用途 |
|------|---------|------|
| **開発** | `dev.platform.prance.co.jp` | 開発・テスト |
| **ステージング** | `staging.platform.prance.co.jp` | 本番前検証 |
...
- `platform.prance.co.jp` のみを[Route 53](docs/GLOSSARY.md#route-53)に委譲
- ルートドメイン `prance.co.jp` はお名前.comで継続管理
...
**必要な作業:**
1. Route 53で `platform.prance.co.jp` のHosted Zoneを作成
...
export const getConfig = (environment: string) => {
  // dev: dev.platform.prance.co.jp
  // staging: staging.platform.prance.co.jp
  // production: platform.prance.co.jp
};
```
...
```bash
# 1. Route 53 Hosted Zone作成（platform.prance.co.jp）
...
# 2. ネームサーバー確認
aws route53 list-hosted-zones-by-name --dns-name prance.co.jp
...
# ブラウザで証明書確認
openssl s_client -connect dev.platform.prance.co.jp:443 -servername dev.platform.prance.co.jp
```
...
- **対応ドメイン:**
  - メインドメイン: `*.platform.prance.co.jp`
  - API: `api.*.platform.prance.co.jp`
  - WebSocket: `ws.*.platform.prance.co.jp`
...
```typescript
allowedOrigins: [
  'https://dev.platform.prance.co.jp',      // 開発
  'https://staging.platform.prance.co.jp',   // ステージング
  'https://platform.prance.co.jp',           // 本番
]
```
