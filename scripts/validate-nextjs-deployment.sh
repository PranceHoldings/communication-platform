...
EXPECTED_API_URL="https://api.app.prance.jp"
if [ "$ENVIRONMENT" != "production" ]; then
  EXPECTED_API_URL="https://api.dev.app.prance.jp"
fi

echo "  NEXT_PUBLIC_API_URL: $API_URL"
if [ "$API_URL" = "$EXPECTED_API_URL" ]; then
  pass "API URL が正しく設定されています"
else
  fail "API URL が不正: $API_URL (期待: $EXPECTED_API_URL)"
  echo "  → NEXT_PUBLIC_API_URL は Next.js ビルド時に HTML に焼き込まれます"
  echo "    Lambda の環境変数だけ変更しても反映されません。再ビルドが必要です。"
fi

# バンドル内の API URL を確認（NEXT_PUBLIC_* はバンドルに焼き込まれる）
# Lambda パッケージが存在する場合のみチェック
if [ -d "/tmp/nextjs-lambda-package/apps/web/.next/static/chunks" ]; then
  BUNDLE_DEV_COUNT=$(grep -rl "ffypxkomg1\|execute-api.*\.amazonaws\.com.*dev" \
    /tmp/nextjs-lambda-package/apps/web/.next/static/chunks/ 2>/dev/null | wc -l | tr -d ' ')
  BUNDLE_PROD_COUNT=$(grep -rl "api\.app\.prance\.jp" \
    /tmp/nextjs-lambda-package/apps/web/.next/static/chunks/ 2>/dev/null | wc -l | tr -d ' ')
  if [ "$BUNDLE_PROD_COUNT" -gt 0 ]; then
    pass "バンドル内 API URL: 本番エンドポイント (${BUNDLE_PROD_COUNT} ファイル)"
  elif [ "$BUNDLE_DEV_COUNT" -gt 0 ]; then
    fail "バンドル内 API URL が dev を指しています (${BUNDLE_DEV_COUNT} ファイル) — 本番用に再ビルドしてください"
  fi
fi
