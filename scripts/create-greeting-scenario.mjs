#!/usr/bin/env node
/**
 * Creates a test scenario with initial greeting for Stage 3 Part 2 tests
 */
...
async function login() {
  ...
  // Handle StandardAPIResponse structure: data.tokens.accessToken
...
async function createScenarioWithGreeting(accessToken) {
  const scenario = {
...
async function main() {
  try {
    ...
    const scenario = await createScenarioWithGreeting(accessToken);
